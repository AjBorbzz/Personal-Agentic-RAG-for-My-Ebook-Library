...
class ProjectGeneratorRequest(BaseModel):
    goal: str
    target_role: str = Field(default="AI Security Engineer")
    project_level: str = Field(default="intermediate")
...
class ProjectGeneratorSource(BaseModel):
    source_number: int
    score: float
...
class ProjectGeneratorResponse(BaseModel):
    ...
    source_count: int
    sources: list[ProjectGeneratorSource]
...
@router.post("/project-generator", response_model=ProjectGeneratorResponse)
async def generate_project(request: ProjectGeneratorRequest):
    ...
    try:
        retrieval_question = (
            ...
            f"Project level: {request.project_level}\n"
            f"Build duration: {request.duration_weeks} weeks"
        )
        ...
        if not decision.matches:
            ...
            return ProjectGeneratorResponse(
                goal=request.goal,
                project_plan="No relevant ebook chunks were found for this project plan.",
                collection_name=settings.default_collection,
        ...
        project_plan = await generate_text(prompt)
